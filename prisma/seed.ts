...
async function main() {
  ...
  for (const role of roles) {
    for (const mod of modules) {
      ...
      if (isAdmin) {
        ...
        canView = true;
        if (['Courses', 'Modules', 'Sessions'].includes(mod)) { canCreate = true; canEdit = true; }
        if (['Users', 'Roles', 'Permissions', 'Settings'].includes(mod)) { canCreate = false; canEdit = false; }
  ...
  const demoUsers = [
    { mobile: '01700000001', name: 'Admin User', email: 'admin@lms.com', role: adminRole, district: undefined as string | undefined },
